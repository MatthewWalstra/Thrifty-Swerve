...
Thriftybot::SwerveDrive::SwerveDrive(SwerveDriveConfig config) 
{
...
std::string Thriftybot::SwerveDrive::getPreferenceKeyForWheel(int wheel)
{
    return "SwerveDrive/wheel." + Util::sstr(wheel);
}

void Thriftybot::SwerveDrive::setDriveMode(Thriftybot::MotorControllerWrapper::DriveMode driveMode)
{
...
void Thriftybot::SwerveDrive::set(double azimuth, double drive)
{
    for (auto wheel : wheels)
    {
...
void Thriftybot::SwerveDrive::drive(double forward, double strafe, double azimuth)
{
    ...
    // wheel azimuth
    wa.at(0) = std::atan2(b, d) * 0.5 / PI;
    wa.at(1) = std::atan2(b, c) * 0.5 / PI;
    wa.at(2) = std::atan2(a, d) * 0.5 / PI;
    wa.at(3) = std::atan2(a, c) * 0.5 / PI;
...
void Thriftybot::SwerveDrive::stop()
{
    for (auto wheel : wheels)
    {
        wheel->stop();
...
void Thriftybot::SwerveDrive::saveAzimuthPositions()
{
    saveAzimuthPositions(frc::Preferences::GetInstance());
}

void Thriftybot::SwerveDrive::saveAzimuthPositions(frc::Preferences *prefs)
{
    for (int i = 0; i < SwerveDriveConfig::WHEEL_COUNT; i++)
    {
...
void Thriftybot::SwerveDrive::zeroAzimuthEncoders()
{
    zeroAzimuthEncoders(frc::Preferences::GetInstance());
}

void Thriftybot::SwerveDrive::zeroAzimuthEncoders(frc::Preferences *prefs)
{
    for (int i = 0; i < SwerveDriveConfig::WHEEL_COUNT; i++)
    {
...
void Thriftybot::SwerveDrive::outputSmartDashboard()
{
    for (int i = 0; i < SwerveDriveConfig::WHEEL_COUNT; i++)
    {
